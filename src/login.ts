import { ky, printImage, qrcode } from "./deps.ts";
import { displayImgToConsole, sleep } from "./utils/index.ts";

const headers = {
  "User-Agent":
    "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.71 Safari/537.36",
  "client-version": "1.4.0",
  extspam:
    "",
  referer: "https://wx.qq.com/?&lang=zh_CN&target=t",
};

const waitingConfirm = async (response: string) => {
  console.log("logining");
  const userAvatar = response.match(/window.userAvatar = '(.*?)';/)?.[1];
  userAvatar && displayImgToConsole(userAvatar);
  await sleep(5);
};

const check = async (uuid: string): Promise<unknown> => {
  var response = await ky.get(
    "https://login.wx.qq.com/cgi-bin/mmwebwx-bin/login",
    {
      timeout: 99999,
      searchParams: {
        loginicon: true,
        uuid,
        tip: 1,
        r: ~new Date(),
        _: +new Date(),
      },
      headers,
    },
  ).text();
  const statusCode = Number(response.match(/window.code=(\d+);/)?.[1]);

  const processFunction = {
    201: waitingConfirm,
  }[statusCode];
  await processFunction?.(response);

  if (statusCode !== 200) {
    return check(uuid);
  }

  console.log("login success");
};

const getCode = async () => {
  const response = await ky.get("https://login.wx.qq.com/jslogin", {
    headers,
    searchParams: {
      appid: "wx782c26e4c19acffb",
      fun: "new",
      redirect_uri:
        "https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxnewloginpage?mod=desktop",
      lang: "zh_CN",
    },
  }).text();
  const code = response?.match(/[A-Za-z_\-\d]{10}==/)?.[0];
  const qrcodeUrl = `https://login.weixin.qq.com/qrcode/${code}`;
  printImage({ path: qrcodeUrl });
  console.log("scan qrcode to login");
  // console.log(`get code successed: ${code}`);
  await check(code!);
};

getCode();
